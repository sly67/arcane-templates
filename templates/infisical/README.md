...
- `PORT` - Http Port (default: `8078`)
- `ENCRYPTION_KEY` - Encryption Key (default: `c6606cd2daad581c4de96531427aae0d`)
- `AUTH_SECRET` - JWT Key (default: `5lrMXKKWCVocS/uerPsl7V+TX/aaUaI7iDkgl3tSmLE=`)
- `POSTGRES_PASSWORD` - Postgres Password (default: `postgres_password`)
- `POSTGRES_USER` - Postgres User (default: `infisical`)
- `POSTGRES_DB` - Postgres Database (default: `infisical`)
- `DB_CONNECTION_URI` - DB Connection URI (default: `postgres://${POSTGRES_USER}:${POSTGRES_PASSWORD}@db:5432/${POSTGRES_DB}`)
- `SITE_URL` - Website URL (default: `http://localhost:8078`)
